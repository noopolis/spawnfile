...
describe("evaluateAcceptanceProfile", () => {
  ...
  it("2. optional subsystem-remote-4090 skipped with a reason, everything else passes -> accepted", () => {
    const results = allPassingResults(FULL_PROFILE).map((result) =>
      result.checkId === "subsystem-remote-4090"
        ? { ...result, status: "skipped" as const, reason: "4090 box offline for maintenance" }
        : result
    );
    ...
    const remote4090 = verdict.checks.find((check) => check.id === "subsystem-remote-4090");
    expect(remote4090?.status).toBe("skipped");
    expect(remote4090?.blocking).toBe(false);
  });
  ...
  it("5. an optional skip without a reason blocks acceptance (R4)", () => {
    ...
    expect(verdict.verdict).toBe("not_accepted");
    expect(verdict.blockers).toContain(
      "optional check skipped without reason: subsystem-remote-4090"
    );
  });
  ...
  it("6. an optional failure blocks acceptance (R5 — optionality licenses absence, not failure)", () => {
    ...
    expect(verdict.verdict).toBe("not_accepted");
    expect(verdict.blockers).toContain(
      "optional check failed: subsystem-remote-4090; optionality licenses absence, not failure"
    );
  });
  ...
  it("9. contract: slice has 10 required checks, full has 28 with remote-4090 as the sole optional", () => {
  ...
  it("A2: a missing optional check is treated as an unexplained skip and blocks", () => {
    const results = allPassingResults(FULL_PROFILE).filter(
      (result) => result.checkId !== "subsystem-remote-4090"
    );
    ...
    expect(verdict.verdict).toBe("not_accepted");
    expect(verdict.blockers).toContain(
      "optional check missing: subsystem-remote-4090; silence is not a reason"
    );
    const remote4090 = verdict.checks.find((check) => check.id === "subsystem-remote-4090");
    expect(remote4090?.status).toBe("missing");
    expect(remote4090?.blocking).toBe(true);
  });
