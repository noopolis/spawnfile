import { renderPiControlWakeIdentitySource } from "./appControlWakeIdentitySource.js";

export const renderPiControlSource = (): string => String.raw`const rebuildAgentKeys = (agents) => {
  const keys = new Map();
...
${renderPiControlWakeIdentitySource()}
const validateDeliveryMetadata = (payload) => {
  if (
    ...
    typeof payload.context_id !== "string" || payload.context_id.trim().length === 0
  ) {
    return null;
  }
  return {
...
const recordControlWakeAccepted = async (targetAgentId, wakeKind, runtimeHomePath, requestId, causeEventIds) => {
  try {
 ...
 */
const recordDeliveryWakeAccepted = async (targetAgentId, wakeKind, runtimeHomePath, requestId, causeEventIds) => {
  try {
 ...
 */
const executeAgentWake = async (agent, payload, requireDeliveryMetadata, agentsByKey) => {
  const deliveryMetadata = requireDeliveryMetadata ? validateDeliveryMetadata(payload) : null;
  if (requireDeliveryMetadata && !deliveryMetadata) {
    throw new Error(DELIVERY_METADATA_INVALID_ERROR);
  }
  const deliveryTarget = requireDeliveryMetadata ? agentsByKey.get(deliveryMetadata.target) : null;
  if (requireDeliveryMetadata && deliveryTarget !== agent) {
    throw new Error(DELIVERY_METADATA_INVALID_ERROR);
  }

  const eventId = requireDeliveryMetadata ? deliveryMetadata.eventId : formatControlEventId(payload, "message-");
  if (typeof payload.message !== "string" || payload.message.trim().length === 0) {
    return { eventId: undefined, executed: false, from: agent.config.id, message: "", wakeKind: undefined };
  }
  ...
  const message = await agent.wake({
    ...
    ...(deliveryMetadata
      ? {
          delivery: {
            eventId: deliveryMetadata.eventId,
            sender: deliveryMetadata.sender,
            target: agent.config.id,
            contextId: deliveryMetadata.contextId
          },
          transportText: typeof payload.transport_text === "string"
  ...
  return { eventId, executed: true, from: agent.config.id, message, wakeKind };
};
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && operatorWakeMatch) {
      ...
      try {
        ...
        if (result.executed) {
          await recordControlWakeAccepted(
            key,
            result.wakeKind,
            agent.paths.runtimeHomePath,
            requestId,
            causeEventIdsForWake(result.wakeKind, result.eventId)
          );
        }
    ...
    try {
      const payload = await readRequestJson(request);
      const result = await executeAgentWake(deliveryAgent, payload, true, agentsByKey);
      if (result.executed) {
        await recordDeliveryWakeAccepted(
          deliveryKey,
          result.wakeKind,
          deliveryAgent.paths.runtimeHomePath,
          deliveryRequestId,
          causeEventIdsForWake(result.wakeKind, result.eventId)
        );
      }
