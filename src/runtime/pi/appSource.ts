export const renderPiApp = (): string => String.raw`import path from "node:path";
...
const registerAgentKeys = (map, agent) => { map.set(agent.config.id, agent); map.set(agent.config.slug, agent); map.set(agent.config.name, agent); };
