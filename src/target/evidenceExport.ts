...
interface Options { readonly authorityStore: EvidenceExportAuthorityStore; readonly context: string; readonly executor: DockerResourceExecutor; readonly exportExecutor: DockerEvidenceExportExecutor; readonly helperArtifactBundle: HelperArtifactBundle; readonly helperArtifactManifestDigest: string; readonly helperArtifactContract: typeof EVIDENCE_EXPORT_HELPER_CONTRACT; readonly artifactIdentityStore: DockerArtifactIdentityStore; readonly journal: TargetJournalStore; readonly signal?: AbortSignal; readonly timeoutMs: number; readonly testHooks?: EvidenceExportTestHooks; }
...
const helperFor = async (value: ExportContext, input: Options): Promise<EvidenceExportHelper> => {
  const bundle = input.helperArtifactBundle; const binding = await input.artifactIdentityStore.resolveOperation(bundle.operationHandle, bundle.requestDigest); const journal = await input.journal.read();
