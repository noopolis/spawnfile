...
export const renderDaimonBrokerProvisioning = (plans: RuntimeTargetPlan[]): string[] => {
  ...
  const program = [
    ...
    "fs.mkdirSync('/etc/daimon-engine-broker', { recursive: true, mode: 0o555 });",
    ...
    `const deniedFor = (entry) => registrations.filter((peer) => peer.uid !== entry.uid).flatMap((peer) => [peer.home, peer.workspace]).concat(['${DAIMON_BROKER_REALM}', '/var/lib/spawnfile/daimon/grok-bootstrap-auth', '/run/daimon-engine-broker', '/var/lib/spawnfile/instances/daimon/daimon-organization/state']);`,
