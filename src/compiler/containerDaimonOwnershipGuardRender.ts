...
export const renderDaimonOwnershipProgram = (
  ...
  // broker realm and usage ledger at broker/broker:organization uid), so —
  // exactly like SPAWNFILE_PRIVATE_STATE_ROOT above — it must stay a shared,
  // root-owned, universally traversable ancestor, never chowned to any one
  // child's uid. Docker always creates it fresh as root:root when it first
  // provisions any volume mounted under it (guaranteed for every Daimon
  // organization now that the usage ledger is provisioned unconditionally:
  // `renderDaimonUsageLedgerProvisioning`), so the plain fixed-traversal
  // preimage — never the organization uid — is the only state this needs to
  // tolerate.
  `secureFixedTraversalAncestor('${SPAWNFILE_PRIVATE_STATE_ROOT}/daimon');`,
