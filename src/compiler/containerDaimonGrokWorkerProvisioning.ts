...
export const renderDaimonGrokWorkerProvisioning = (
  ...
  "for (const entry of grokWorkers) for (const denied of entry.denyPaths) { if (entry.deferredDenyPaths.includes(denied)) { try { fs.lstatSync(denied); } catch (error) { if (error.code !== 'ENOENT') throw error; fs.mkdirSync(denied, { mode: 0o755, recursive: true }); } } assertPlaceable(denied, entry.uid, entry.uid); }",
