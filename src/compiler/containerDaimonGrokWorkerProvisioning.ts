...
export const renderDaimonGrokWorkerProvisioning = (
  ...
  // Deny-path placement, asserted once every mode above is final. A resource backing the organization
  // entrypoint only materializes later is created here, empty and root-owned, rather than skipped: the
  // engine broker starts before that entrypoint runs and its own placement preflight refuses a deny target
  // that does not exist, which exits the container 1 before any agent attaches. `prepare_bundle_resource`
  // and `prepare_git_resource` both treat an empty backing as unmaterialized, so the content still lands.
  // Grok 1.0.34 materializes each deny
  ...
  // A deferred backing usually sits under a resource instance directory the ownership pass has already
  // handed to the organization uid, so root has to borrow each missing ancestor to create inside it and
  // hand it straight back: the entrypoint that materializes the resource depends on those modes.
  `const createDeferredDenyTarget = (target) => { const parent = require('node:path').dirname(target); let info; try { info = fs.lstatSync(parent); } catch (error) { if (error.code !== 'ENOENT') throw error; createDeferredDenyTarget(parent); info = fs.lstatSync(parent); } if (!info.isDirectory() || info.isSymbolicLink()) throw new Error(\`unsafe deferred deny parent: \${parent}\`); const mode = info.mode & 0o7777, borrowed = info.uid !== 0 || (mode & 0o300) !== 0o300; if (borrowed) { fs.chownSync(parent, 0, 0); fs.chmodSync(parent, 0o700); } try { fs.mkdirSync(target, { mode: 0o755 }); fs.chmodSync(target, 0o755); fs.chownSync(target, ${DAIMON_ORGANIZATION_UID}, ${DAIMON_ORGANIZATION_UID}); } finally { if (borrowed) { fs.chmodSync(parent, mode); fs.chownSync(parent, info.uid, info.gid); } } };`,
  ...
  "for (const entry of grokWorkers) for (const denied of entry.denyPaths) { if (entry.deferredDenyPaths.includes(denied)) { try { fs.lstatSync(denied); } catch (error) { if (error.code !== 'ENOENT') throw error; createDeferredDenyTarget(denied); } } assertPlaceable(denied, entry.uid, entry.uid); }",
