...
async function assortedTree(root: string): Promise<void> {
  const files: [string, number][] = [["grok", 0o700], ["train", 0o600], ["bridge/requirements.lock", 0o600], ["bridge/pkg/tool.py", 0o640],
...
it("stages COPY content with the modes the former chmod 0555 grok/train plus chmod -R a+rX produced", async () => {
  const root = await temporary(), reference = path.join(root, "reference"), candidate = path.join(root, "candidate");
  await assortedTree(reference); await assortedTree(candidate);
  await chmod(path.join(reference, "grok"), 0o555); await chmod(path.join(reference, "train"), 0o555);
  execFileSync("chmod", ["-R", "a+rX", reference]);
  ...
  // Documented expectation: directories 0755, readable files 0644, executables 0755, grok/train 0555.
  expect(expected).toMatchObject({ bridge: "755", "bridge/pkg": "755", "bridge/requirements.lock": "644", "bridge/pkg/run.sh": "755",
    "integration/entry.ts": "444", "integration/bin/partial": "555", grok: "555", train: "555", sticky: "2755", "sticky/file": "4755" });
...
it("hands Docker a normalized context and never runs a layer after the distribution copies", async () => {
  ...
  await chmod(path.join(f.root, "integration/entry.ts"), 0o600); await chmod(path.join(f.root, "grok"), 0o700);
  ...
  expect(staged).toMatchObject({ grok: "555", train: "555", integration: "755", "integration/entry.ts": "644", bridge: "755", "paideia/dist/src/cli": "755" });
  ...
  const firstLate = instructions.indexOf("COPY grok /opt/training/bin/grok");
  ...
  expect(copies.flatMap(line => line.split(/\s+/u).slice(1, -1)).sort()).toEqual(["bootstrap", "bridge", "bridge/requirements.lock", "claude/package-lock.json", "claude/package.json",
    "compiler/dist", "compiler/moltnet-releases.json", "compiler/package-lock.json", "compiler/package.json", "compiler/runtimes.yaml", "grok", "integration",
    ...
    "spawnfile/package.json", "spawnfile/runtimes.yaml", "train"]);
