...
For managed SQLite or JSON stores, Spawnfile treats persistence as durable unless you set `store.persistence.mode: ephemeral`. Durable stores produce a Docker named volume when you use `spawnfile run` or `spawnfile up`. That volume is reattached, not recreated: its name depends on the project and deployment, never on the run id, so message history survives removing and recreating the container. A `store.persistence.name` you declare is used verbatim as the host volume name. Open Moltnet registration tokens are stored as per-agent private runtime state and are mounted the same way, so claimed agent IDs survive container replacement. The same holds for `kind: volume` workspace resources. Because each of these volumes may have only one live holder, an organization that declares any of them replaces its deployment in place instead of running a concurrent canary.
