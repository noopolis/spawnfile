...
import { createSourceBundle } from "./source-provenance-bundle.ts";
import type { SourceBundleProfile } from "./source-provenance-bundle.ts";
...
export const main = (argv = process.argv.slice(2)): void => {
  ...
  const receipt = createSourceBundle(values[0] ?? "", values[1] ?? "", profile as SourceBundleProfile);
