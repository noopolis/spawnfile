...
import type { TrackedSourceEntry } from "./build-local-moltnet.ts";
import { validateSourceBundle } from "./source-provenance-bundle.ts";
import type { NodeDependencyLock, SourceBundleProfile, SourceBundleReceipt } from "./source-provenance-bundle.ts";

type DaimonBuildArchitecture = "amd64" | "arm64";
type SupportedHostArchitecture = "arm64" | "x64";
type DigestAlgorithm = "sha256" | "sha512";

type DaimonCliArtifacts = {
  agy: {
...
type SourceBundleWithPath = SourceBundleReceipt & {
  path: string;
};

type BundleBuiltDaimon = {
  packagePath: string;
  sourceInputs: DaimonSourceInputs;
  sourceSha256: string;
};
...
type CapabilityReceiptOptions = {
  architecture: DaimonBuildArchitecture;
  artifacts: DaimonCliArtifacts;
...
const requiredDigest = (env: Record<string, string | undefined>, name: string, algorithm: DigestAlgorithm, pattern: RegExp): string => {
...
const requiredUrl = (env: Record<string, string | undefined>, name: string): string => {
  const value = env[name]?.trim();
  let parsed;
  try {
  ...
  if (!value) throw new Error(`${name} must be a credential-free HTTPS URL without query or fragment`);
  return value;
};

const requiredVersion = (env: Record<string, string | undefined>, name: string): string => {
  const value = env[name]?.trim();
  if (!value || !version.test(value)) throw new Error(`${name} must be an explicit artifact version`);
  return value;
};

export const readDaimonCliArtifactPins = (env: Record<string, string | undefined> = process.env): DaimonCliArtifacts => ({
  agy: {
...
export const resolveLocalImageTag = (value: string | undefined): string => {
  ...
  if (!tag || !Number.isInteger(port) || port < 1 || port > 65_535) throw new Error("SPAWNFILE_DAIMON_LOCAL_IMAGE_TAG must use an explicit 127.0.0.1 loopback registry port");
...
export const resolveLocalBuildArchitecture = (hostArchitecture: string, requested = process.env.SPAWNFILE_DAIMON_TARGET_ARCH): DaimonBuildArchitecture => {
...
export const resolvePushedImageReference = (imageTag: string, repoDigests: string[]): string => {
  ...
  if (matches.length !== 1 || !match || !new RegExp(`^${expectedPrefix.replaceAll(".", "\\.")}sha256:[a-f0-9]{64}$`, "u").test(match)) {
...
const stagePackagedDaimon = (directory: string): string => {
...
const requiredBundle = (envName: string, stagedName: string, directory: string, expectedProfile: SourceBundleProfile): SourceBundleWithPath => {
...
export const resolveDaimonSourceMode = (env: Record<string, string | undefined> = process.env): "clean-git" | "source-bundle" => {
...
const stageOfflineCliAssets = (directory: string, artifacts: DaimonCliArtifacts): void => {
  for (const [envName, destination, expected, algorithm] of [["SPAWNFILE_AGY_CLI_ARCHIVE", "agy.tar.gz", artifacts.agy.archive_sha512, "sha512"], ["SPAWNFILE_GROK_CLI_FILE", "grok", artifacts.grok.executable_sha256, "sha256"]] as const) {
...
const nodeDependencyLock = (lock: SourceBundleReceipt["manifest"]["dependency_lock"]): NodeDependencyLock => {
  if (!lock || !("package_lock_sha256" in lock)) throw new Error("Dependency bundle lacks package-lock identity");
  return lock;
};

const stageBundleBuiltDaimon = (directory: string, artifacts: DaimonCliArtifacts, architecture: DaimonBuildArchitecture): BundleBuiltDaimon => {
  ...
  const dependencyLock = nodeDependencyLock(dependencies.manifest.dependency_lock);
  const sourceInputs = { dependencies: { archive_sha256: dependencies.archive_sha256, manifest_sha256: dependencies.manifest_sha256,
      package_lock_sha256: dependencyLock.package_lock_sha256, runtime_archive_sha256: runtimeArchiveSha256 },
    mode: "source-bundle", source: { archive_sha256: source.archive_sha256, manifest_sha256: source.manifest_sha256 },
    version: "spawnfile.daimon-source-inputs.v1" } satisfies DaimonSourceInputs;
  const buildIdentity = JSON.parse(readFileSync(path.join(output, "source-inputs.json"), "utf8")) as BuildSourceIdentity;
...
export const createLocalDaimonCapabilityReceipt = ({ architecture, artifacts, manifestSha256, packageSha256, sourceInputs, sourceSha256 }: CapabilityReceiptOptions) => ({
  architecture,
...
const main = (): void => {
  ...
  try {
    ...
    const receiptOptions: CapabilityReceiptOptions = {
      architecture,
      artifacts,
    ...
    if (bundled) receiptOptions.sourceInputs = bundled.sourceInputs;
    const receipt = createLocalDaimonCapabilityReceipt(receiptOptions);
    ...
    if (!imageConfigDigest || !/^sha256:[a-f0-9]{64}$/u.test(imageConfigDigest)) throw new Error("Docker did not return an immutable image config digest");
    ...
    if (!repoDigestsJson) throw new Error("Docker did not return local Daimon repository digests");
    const imageReference = resolvePushedImageReference(imageTag, JSON.parse(repoDigestsJson) as string[]);
