...
const docker = async (args: string[]) => await execFile("docker", args);

const isExecErrorWithStdout = (error: unknown, stdout: string): boolean => (
  error !== null
  && typeof error === "object"
  && "stdout" in error
  && error.stdout === stdout
...
for (const architecture of ["x64", "arm64"] as const) test(`real Linux ${architecture} helper atomically activates and preserves EEXIST destinations`, async () => {
  ...
  try {
    ...
    for (const nonempty of [false, true]) {
      ...
      await assert.rejects(
        docker(["exec", container, "/rename-noreplace", "/work", `source-${suffix}`, `destination-${suffix}`]),
        (error) => isExecErrorWithStdout(error, '{"ok":false,"error":"EEXIST","errno":17}\n'),
      );
